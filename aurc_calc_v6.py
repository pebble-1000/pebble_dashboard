...
def tab_km_app(df):
    ...
    with c3:
        st.metric("ΔAURC / 개선율", f"{dA:+.2f}", f"{ratio:+.2f}%")
    ...
    # 5) 운영 임팩트 (₩ 추정)
    st.markdown("<br>", unsafe_allow_html=True)
    st.subheader("5) 운영 임팩트 (₩ 추정)")

    st.caption("""
    운영 임팩트는 ΔAURC를 기반으로, 향후 누적 추가가치를 원화(₩)로 환산한 값입니다.  
    다음 3가지 변수를 입력받아 계산합니다:
    - ΔAURC: 이번 개선으로 평균 잔존기간이 몇 개월 늘어났는가
    - 월 신규 수업 수: 한 달에 새로 시작하는 수업 수 (이 개선 효과가 적용된다고 가정)
    - 수업 1건당 LTV: 수업 하나가 생애 전체에서 만드는 평균 가치(매출 또는 기여이익 기준)

    아래 결과는 '이번 달에 유입된 신규 코호트 1개월치'가 앞으로 만들어낼 누적 추가가치(원)입니다.
               
    계산식 = ( ΔAURC (개월)  ×  월 신규 수업 수 (건) )  ×  수업 1건당 LTV (원)
    """)

    col1, col2, col3 = st.columns(3)

    with col1:
        monthly_new_classes = st.number_input(
            "월 신규 수업 수 (건)",
            min_value=0,
            value=1500,
            step=1,
            help="이번 달 신규로 시작하는 수업 수 (코호트 크기)"
        )

    with col2:
        ltv_per_class_input = st.number_input(
            "수업 1건당 LTV (원)",
            min_value=0,
            value=120000,
            step=1000,
            help="수업 한 건이 생애 전체 동안 만들어내는 평균 가치(매출/기여이익 등 내부 정의값)"
        )

    with col3:
        st.metric("ΔAURC (개월)", f"{dA:+.2f}")

    # 월 추가 누적가치 계산
    total_monthly_uplift_simple = ((dA * monthly_new_classes) * ltv_per_class_input)

    st.metric(
        "월 추가 누적가치 (원)",
        f"{total_monthly_uplift_simple:,.0f} 원",
        help="이번 달 신규로 시작하는 1개월치 신규 코호트가, 향후 생애 전체에서 추가로 창출할 누적가치 규모(원)"
    )
