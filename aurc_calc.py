...
def tab_churn(df):
    ...
    cycle_col    = _pick_col(df, ["cycle_count"])

    # 7개 모두 필수 컬럼
    required_cols = {
        "lvt(lecture_vt_No)": _pick_col(df, ["lecture_vt_No"]),
        "첫 결제일(fst_pay_date)": _pick_col(df, ["fst_pay_date"]),
        "과외상태(tutoring_state)": _pick_col(df, ["tutoring_state"]),
        "done_month": _pick_col(df, ["done_month"]),
        "단계(step)": _pick_col(df, ["step"]),
        "이탈여부(reactiveornot)": _pick_col(df, ["reactiveornot"]),
        "cycle_count": _pick_col(df, ["cycle_count"]),
    }
    ...
    if reactive_col is not None and reactive_col in df.columns:
        raw_total = int((df[reactive_col].astype(str) != "T").sum())
    else:
        raw_total = len(df)
...
def tab_km_app(df):
    ...
    if is_week:
        ...
        H = st.number_input("Horizon (개월)", min_value=6, max_value=60, value=12, step=1, key="km01_H_month")
...
##=====================DM1을 주차 세분화 하기=========================
    ...
    c1, c2, c3 = st.columns(3)
    with c1: st.metric("Baseline AURC", f"{A0:.2f}")
    with c2: st.metric("Scenario AURC", f"{A1:.2f}")
    with c3: st.metric("ΔAURC / 개선율", f"{dA:+.2f}", f"{ratio:+.1f}%")
